...
from models.timesformer_model import TimeSformerClassifier
...
batch_size = 2  # TimeSformer는 메모리 소모 커서 줄임
epochs = 10
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
...
for epoch in range(epochs):
    ...
    # 모델 저장
    torch.save(model.state_dict(), f"weights/timesformer_epoch{epoch+1}.pth")

# 그래프 저장
save_metrics_plot(train_loss_list, train_acc_list, "metrics/timesformer_train_metrics.png")
save_metrics_plot(val_loss_list, val_acc_list, "metrics/timesformer_val_metrics.png")
print("🔵 학습 완료!")
